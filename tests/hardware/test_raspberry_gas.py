...
def test_gas_sensor_configures_input():
    fake, sensor = create_sensor()

    assert fake.configured[17] == fake.IN


def test_active_low_detects_alert_when_low():
    fake, sensor = create_sensor(active_low=True)

    fake.inputs[17] = fake.LOW

    assert sensor.is_alert() is True


def test_active_low_detects_safe_when_high():
    fake, sensor = create_sensor(active_low=True)

    fake.inputs[17] = fake.HIGH

    assert sensor.is_alert() is False


def test_active_high_detects_alert_when_high():
